...
export default function RecentlyViewed({ currentProductId }: { currentProductId?: string }) {
  ...
  return (
    <div className="border-t border-[#cbcbcb] px-6 md:px-10 py-12 bg-white">
      <div className="flex items-center justify-between mb-8">
        <h2 className="text-sm tracking-[0.3em] font-light uppercase">Recently Viewed</h2>
        <div className="flex gap-4">
          <button 
            onClick={() => scroll("left")}
            className="w-10 h-10 border border-[#cbcbcb] flex items-center justify-center hover:bg-black hover:text-white transition-colors"
          >
            <ChevronLeft size={16} />
          </button>
          <button 
